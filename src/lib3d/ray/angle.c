...
t_angle	get_angle_to(t_FVec3 dir, t_FVec3 desired_angle)
{
	t_FVec3	rot_obj;
	t_angle	new_angle;
	float	len_rot_obj;

	if (ft_equal_fvec3(dir, ft_mul_fvec3(desired_angle, -1)))
	{
		new_angle.angle = M_PI;
		if (desired_angle.y == 0 && desired_angle.z == 0)
			new_angle.k = ft_normalize_fvec3(ft_cross_fvec3( \
	...
	else
	{
		...
		new_angle.k = ft_normalize_fvec3(rot_obj);
	}
	return (new_angle);
}
